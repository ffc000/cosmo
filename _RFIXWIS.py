...
def generar_html(registros: list, fecha_max: str, serie: list = None, historial_lotes: dict = None) -> str:
    ...
    historial_js_str = _json.dumps(historial_lotes or {}, ensure_ascii=False)
...
var HISTORIAL = {{historial_js_str}};
...
function cargarEvolucion() {{
  ...
  var key = d.adu + '-' + d.lot;
  var serie = HISTORIAL[key];

  document.getElementById('btn-evolucion').style.display = 'none';

  if(!serie || serie.length === 0) {{
    var lbl = document.getElementById('evol-loading');
    lbl.textContent = 'Sin historial previo disponible para este depósito.';
    lbl.style.display = 'block';
    return;
  }}

  renderizarEvolucion(serie);
}}
