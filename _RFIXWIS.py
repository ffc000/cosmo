...
def generar_html(registros: list, fecha_max: str, serie: list = None, tendencia: dict = None) -> str:
    ...
    for idx, r in enumerate(registros):
        ...
        treg_pct    = td.get('pct_reg',   '—')
        tstock_pct  = td.get('pct_stock', '—')
        treg_sym    = td.get('treg',   '')
        tstock_sym  = td.get('tstock', '')
        treg_cell   = f"{treg_pct} {treg_sym}".strip() if treg_pct != '—' else '—'
        tstock_cell = f"{tstock_pct} {tstock_sym}".strip() if tstock_pct != '—' else '—'
        treg_tip    = f"F.Reg: {td.get('freg_t',0)} transmitió · {td.get('freg_nt',0)} no transmitió · {td.get('freg_nh',0)} no hábil"
        tstock_tip  = f"F.Stock: {td.get('fstock_t',0)} transmitió · {td.get('fstock_nt',0)} no transmitió · {td.get('fstock_nh',0)} no hábil"
        filas_html.append(
            ...
            f'<td style="text-align:center;font-size:.82rem;white-space:nowrap" title="{treg_tip}">{treg_cell}</td>'
            f'<td style="text-align:center;font-size:.82rem;white-space:nowrap" title="{tstock_tip}">{tstock_cell}</td>'
...
thead th:nth-child(9){{width:11%}}
thead th:nth-child(10){{width:4%}}
thead th:nth-child(11){{width:4%}}
thead th:nth-child(12){{width:11%}}
  ...
  <table id="tabla">
    <thead>
      <tr>
        ...
        <th title="% cumplimiento F.Registro (días hábiles) y tendencia vs reporte anterior">Reg %</th>
        <th title="% cumplimiento F.Stock (días hábiles) y tendencia vs reporte anterior">Stock %</th>
        <th>Observación</th>
      </tr>
    </thead>
    <tbody id="tbody">
...
function abrirModal(idx) {{
  ...
  var fregLine   = 'F. Registro: ' + (d.freg_t||0) + ' transmitió · ' + (d.freg_nt||0) + ' no transmitió · ' + (d.freg_nh||0) + ' no hábil' +
    ' — <strong>' + (d.pct_reg||'—') + '</strong>' + (d.treg && d.treg!='—' ? ' ' + d.treg : '');
  var fstockLine = 'F. Stock: '     + (d.fstock_t||0) + ' transmitió · ' + (d.fstock_nt||0) + ' no transmitió · ' + (d.fstock_nh||0) + ' no hábil' +
    ' — <strong>' + (d.pct_stock||'—') + '</strong>' + (d.tstock && d.tstock!='—' ? ' ' + d.tstock : '');
  document.getElementById('modal-sub').innerHTML =
    ...
    '<br><span style="font-size:.75rem;color:#374151;margin-top:.4rem;display:block;line-height:1.6">' +
    fregLine + '<br>' + fstockLine + '</span>';
