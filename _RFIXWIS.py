...
class RefAduanasNoDisponibleError(Exception):
    """La tabla ref_aduanas o ref_dira no existe o está vacía en la BD. No hay fallback
    hardcodeado: ambas se administran desde el panel de administración (Ref. Aduanas / DIRA)."""
    pass
...
def _cargar_ref_aduanas_desde_bd():
    ...
    Lanza RefAduanasNoDisponibleError si la tabla no existe, está vacía, o la BD no es accesible."""
    try:
        con = _sq3_ref.connect("/data/historial.db")
        con.row_factory = _sq3_ref.Row
        existe = con.execute(
