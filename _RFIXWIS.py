...
def generar_html(registros: list, fecha_max: str, serie: list = None, tendencia: dict = None) -> str:
    ...
    for idx, r in enumerate(registros):
        ...
        treg_tip   = f"F.Reg: {td.get('freg_t',0)}↑ {td.get('freg_nt',0)}✗ {td.get('freg_nh',0)}○"
        tstock_tip = f"F.Stock: {td.get('fstock_t',0)}↑ {td.get('fstock_nt',0)}✗ {td.get('fstock_nh',0)}○"
        filas_html.append(
            ...
            f'<td style="text-align:center;font-size:.9rem" title="{treg_tip}">{treg_sym}</td>'
            f'<td style="text-align:center;font-size:.9rem" title="{tstock_tip}">{tstock_sym}</td>'
    ...
    # Contadores de tendencia para el resumen
    _tend_label = {'↑': 'Mejora', '→': 'Estable', '↓': 'Desmejora', '—': 'Sin prev.'}
    if tendencia:
        _cnt_tend = {'↑': 0, '→': 0, '↓': 0, '—': 0}
        for _td in tendencia.values():
            _k = _td.get('treg', '—')
            _cnt_tend[_k] = _cnt_tend.get(_k, 0) + 1
        resumen_tend = '<span style="color:#d1d5db;margin:0 .75rem;align-self:center">|</span>' + ''.join(
            f'<div class="resumen-chip" title="Tendencia F.Registro: {_tend_label[s]}">'
            f'<span class="resumen-num" style="color:#374151;font-size:1rem">{s} {_cnt_tend[s]}</span>'
            f'<span class="resumen-label-chip">{_tend_label[s]}</span></div>'
            for s in ['↑', '→', '↓', '—']
        )
    else:
        resumen_tend = ''
  ...
  <table id="tabla">
    <thead>
      <tr>
        ...
        <th title="Tendencia F.Registro vs reporte anterior">T.Reg</th>
        <th title="Tendencia F.Stock vs reporte anterior">T.Stock</th>
        <th>Observación</th>
      </tr>
    </thead>
    <tbody id="tbody">
...
function abrirModal(idx) {{
  ...
  document.getElementById('modal-sub').innerHTML =
    ...
    '<br><span style="font-size:.72rem;color:#6b7280;margin-top:.3rem;display:block">' +
    'T.Reg: ' + (d.treg||'—') + ' (' + (d.freg_t||0) + '↑ ' + (d.freg_nt||0) + '✗ ' + (d.freg_nh||0) + '○)' +
    ' &nbsp;·&nbsp; T.Stock: ' + (d.tstock||'—') + ' (' + (d.fstock_t||0) + '↑ ' + (d.fstock_nt||0) + '✗ ' + (d.fstock_nh||0) + '○)' +
    '</span>';
