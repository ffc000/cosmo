...
function abrirModal(idx) {{
  ...
  var thead = '<tr><th style="min-width:70px"></th>';
  for(var i=0;i<15;i++) {{
    var esFer = esFeriado(FECHAS[i]);
    var thStyle = esFer ? 'background:#1a56db;color:#fff' : '';
    thead += '<th style="min-width:40px;' + thStyle + '">' + DIAS[i] + '<br>' + FECHAS[i] + '</th>';
  }}
  thead += '</tr>';
  ...
  document.getElementById('modal-15').innerHTML =
    '<table style="border-collapse:collapse;width:100%;min-width:750px"><thead>' + thead + '</thead><tbody>' + fregRow + fstockRow + '</tbody></table>';
