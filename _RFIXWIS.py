...
function filtrar() {{
  ...
  rows.forEach(function(r) {{
    // Buscar en ADU(0), LOT(1), Razón Social(2), CUIT(3), Dir. Regional(4)
    var txt = [0,1,2,3,4].map(function(i){{ return r.cells[i]?r.cells[i].textContent.toLowerCase():''; }}).join(' ');
...
function exportarExcel() {{
  ...
  var data = [['ADU','LOT','Razón Social','CUIT','Dir. Regional','Estado','Últ. F.Registro','Últ. F.Stock','Observación']];
  filas.forEach(function(r) {{
    var idx = parseInt(r.dataset.idx);
    var d = DATOS[idx];
    data.push([d.adu, d.lot, d.razon, d.cuit, d.dira, d.sem, d.freg||'', d.fstock||'', d.coment||'']);
  }});
