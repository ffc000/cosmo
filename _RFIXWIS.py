...
def generar_html(registros: list, fecha_max: str, serie: list = None) -> str:
    ...
    resumen_items = ''.join(
        f'<div style="display:inline-block;margin:0 10px;text-align:center;cursor:pointer" '
        f'onclick="filtrarEstado(\'{s}\')" title="{s}">'
        f'<span style="display:block;font-size:1.8rem;font-weight:700;color:{COLOR_MAP[s]}">{conteo[s]}</span>'
        f'<span style="font-size:.72rem;color:#888">({pct(conteo[s])})</span></div>'
...
table{{width:100%;border-collapse:collapse;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.07);border-radius:6px;overflow:hidden;table-layout:fixed}}
...
thead th:nth-child(9){{width:14%}}
thead th:nth-child(10){{width:13%}}
...
.modal-box{{background:#fff;border-radius:10px;padding:1.5rem;max-width:1200px;width:90vw;box-shadow:0 8px 32px rgba(0,0,0,.18);position:relative}}
...
.modal-15 table{{font-size:.72rem;width:100%;border-collapse:collapse}}
.modal-15 th{{background:#1E2A3B;color:#fff;padding:.4rem .3rem;text-align:center;font-size:.65rem}}
.modal-15 td{{padding:.35rem .3rem;text-align:center;border-bottom:1px solid #f0f0f0;font-size:.65rem}}
...
<div class="container">
  <div class="search-bar">
    ...
    <button onclick="abrirGrafico()" style="font-size:.75rem;padding:.35rem .7rem;border:1px solid #1A56DB;border-radius:4px;background:#EBF5FF;color:#1A56DB;cursor:pointer;font-weight:600">📈 Gráfico</button>
  </div>
  <table id="tabla">
    <thead>
      <tr>
...
<div class="modal-overlay" id="modal-grafico-overlay" onclick="if(event.target===this)cerrarGrafico()">
  <div style="background:#fff;border-radius:10px;padding:1.5rem;max-width:1200px;width:90vw;box-shadow:0 8px 32px rgba(0,0,0,.18);position:relative">
...
function esFeriado(fechaStr) {{
  ...
  // Detecta sáb/dom por día de semana
...
function abrirModal(idx) {{
  ...
  // Tabla 15 días — encabezado con día y fecha en dos líneas
  var thead = '<tr><th style="min-width:80px"></th>';
  for(var i=0;i<15;i++) {{
    var esFer = esFeriado(FECHAS[i]);
    var thStyle = esFer ? 'background:#1a56db;color:#fff' : '';
    thead += '<th style="min-width:48px;' + thStyle + '">' + DIAS[i] + '<br>' + FECHAS[i] + '</th>';
  }}
  thead += '</tr>';

  // Fila Fecha Registro
  var fregRow = '<tr><td style="font-weight:600;white-space:nowrap;padding:.4rem .6rem">Fecha<br>Registro</td>';
  for(var i=0;i<15;i++) {{
    var val = d.u15[i];
    var esFer = esFeriado(FECHAS[i]);
    var color, border='';
    if(val) {{
      ...
      color = '#1a56db';   // azul: no informó pero era feriado/sáb/dom
    }} else {{
      color = '#cc0000';   // rojo: no informó en día hábil
    }}
    fregRow += '<td style="text-align:center"><span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:' + color + '"></span></td>';
  }}
  fregRow += '</tr>';

  // Fila Fecha Stock — usa u15s (marcado por fecha de stock)
  var fstockRow = '<tr><td style="font-weight:600;white-space:nowrap;padding:.4rem .6rem">Fecha<br>Stock</td>';
  for(var i=0;i<15;i++) {{
    var val = d.u15s[i];
    var esFer = esFeriado(FECHAS[i]);
    var color;
    ...
    fstockRow += '<td style="text-align:center"><span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:' + color + '"></span></td>';
  }}
  fstockRow += '</tr>';

  document.getElementById('modal-15').innerHTML =
    '<table style="border-collapse:collapse;width:100%;min-width:900px"><thead>' + thead + '</thead><tbody>' + fregRow + fstockRow + '</tbody></table>';
...
(function() {{
  ...
  var azules   = SERIE.map(function(r){{return r[1];}});
  var negros   = SERIE.map(function(r){{return r[2];}});
  var rojos    = SERIE.map(function(r){{return r[3];}});
  var amarillos= SERIE.map(function(r){{return r[4];}});
  var verdes   = SERIE.map(function(r){{return r[5];}});
  new Chart(ctx, {{
    type: 'bar',
    data: {{
      labels: labels,
      datasets: [
        {{label:'Verde',   data:verdes,    backgroundColor:'#00bf00'}},
        {{label:'Azul',    data:azules,    backgroundColor:'#297ccf'}},
        {{label:'Amarillo',data:amarillos, backgroundColor:'#e6b800'}},
        {{label:'Rojo',    data:rojos,     backgroundColor:'#bf0000'}},
        {{label:'Negro',   data:negros,    backgroundColor:'#444444'}},
      ]
    }},
    options: {{
      responsive: true,
      plugins: {{
        ...
        title: {{display:true, text:'Evolución últimos 15 días', font:{{size:13}}}}
      }},
      scales: {{
...
def calcular_serie_grafico(stock_txt: str, depositos_txt: str,
                           fecha_max: str, dias_tolerancia: int = 0) -> list:
    """
    15 cortes diarios hacia atrás desde fecha_max.
    Devuelve lista de [fecha_dd/mm/yyyy, azul, negro, rojo, amarillo, verde].
    """
    arr_stock = parsear_stock(stock_txt)
    arr_depo  = parsear_depositos(depositos_txt)

    # Reconstruir TXT para reusar procesar()
    hdr_s   = 'Aduana\tLugar Operativo\tFecha del Stock\tFecha de Registro'
    hdr_d   = 'Aduana de residencia\tCodigo del lugar Operativo\tDescripcion\tCUIT de la Terminal/Empresa Concesionaria\tTipo de lugar \tFecha fin vigencia'
    stock_r = hdr_s + '\n' + '\n'.join('\t'.join(r) for r in arr_stock)
    depo_r  = hdr_d + '\n' + '\n'.join('\t'.join(r) for r in arr_depo)
    ...
    for fc in fechas:
        recs = procesar(stock_r, depo_r, fc, dias_tolerancia)
        c = {'VERDE':0,'AZUL':0,'AMARILLO':0,'ROJO':0,'NEGRO':0}
        for r in recs:
            s = r[5]
            if s in c: c[s] += 1
        serie.append([yymmdd_to_ddmmyyyy(fc),
                      c['AZUL'], c['NEGRO'], c['ROJO'], c['AMARILLO'], c['VERDE']])
    return serie
