  ...
  <table id="tabla">
    <thead>
      <tr>
        ...
        <th onclick="sortTable(5)">Dir. Regional ↕</th>
        <th>Estado</th>
...
function filtrar() {{
  ...
  rows.forEach(function(r) {{
    // Buscar en ADU(0), LOT(1), Razón Social(2), CUIT(3), Aduana(4), Dir. Regional(5)
...
function exportarExcel() {{
  ...
  var data = [['ADU','LOT','Razón Social','CUIT','Aduana','Dir. Regional','Estado','Últ. F.Registro','Últ. F.Stock','Reg. últ.15d','Stock últ.15d','Observación']];
  filas.forEach(function(r) {{
    var idx = parseInt(r.dataset.idx);
    var d = DATOS[idx];
    data.push([d.adu, d.lot, d.razon, d.cuit, d.ndu, d.dira, d.sem, d.freg||'', d.fstock||'',
  ...
  // Ancho de columnas
  ws['!cols'] = [4,8,30,14,26,18,10,14,14,14,14,40].map(function(w){{return {{wch:w}}}});
