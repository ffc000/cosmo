...
def _calcular_vua_pendientes(api_key):
    """Cruza minutas VUA con IA para detectar compromisos sin seguimiento.
    Extraído de vua_acuerdos_pendientes() (Fase 9) para poder llamarlo
    también desde el thread de caché del dashboard, no solo desde el
    endpoint. Devuelve el dict de resultado (mismo shape que antes),
    nunca levanta excepción -- el caller decide qué hacer con el error."""
    try:
        import anthropic, httpx
        with get_db(HIST_DB, row_factory=True) as con:
            minutas = [dict(r) for r in con.execute(
        ...
        if len(minutas) < 2:
            return {"ok": True, "pendientes": [],
                    "mensaje": "Se necesitan al menos 2 minutas para detectar pendientes."}
...
def vua_acuerdos_pendientes():
    ...
    api_key = get_api_key()
    if not api_key: return jsonify({"ok": False, "error": "API key no configurada"})
    return jsonify(_calcular_vua_pendientes(api_key))


def _actualizar_vua_pendientes_cache():
    """Corre _calcular_vua_pendientes() en background cada 6hs y guarda el
    resultado en vua_pendientes_cache (migración 004) -- así el widget de
    pendientes del dashboard lee un valor cacheado en vez de llamar a la IA
    en cada carga de página (sería carísimo y lento: una carga del
    dashboard no puede depender de una llamada a Claude)."""
    while True:
        try:
            api_key = get_api_key()
            if api_key:
                resultado = _calcular_vua_pendientes(api_key)
                if resultado.get("ok"):
                    with get_db(HIST_DB) as con:
                        con.execute(
                            "INSERT INTO vua_pendientes_cache (clave,resultado,actualizado) VALUES ('ultimo',?,datetime('now')) "
                            "ON CONFLICT(clave) DO UPDATE SET resultado=excluded.resultado, actualizado=excluded.actualizado",
                            (json.dumps(resultado),))
        except Exception:
            logging.exception("Error actualizando cache de pendientes VUA")
        threading.Event().wait(21600)  # cada 6 horas

threading.Thread(target=_actualizar_vua_pendientes_cache, daemon=True).start()
