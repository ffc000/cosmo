...
def _guardar_reporte_bd(reporte_id, fecha_corte, dias_tol, usuario, registros):
    ...
    con.execute("""INSERT OR REPLACE INTO stock_reportes
        (id, fecha_corte, dias_tol, usuario, total, verde, azul, amarillo, rojo, negro)
        VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (reporte_id, fecha_corte, dias_tol, usuario, total,
         conteo["VERDE"], conteo["AZUL"], conteo["AMARILLO"],
         conteo["ROJO"], conteo["NEGRO"]))
...
def api_stock_generar():
    ...
    try:
        ...
        # Primero persistir, luego leer historial (incluye la corrida actual)
        conteo, total = _guardar_reporte_bd(
            reporte_id, fecha_iso, dias_tol,
            session.get("username", "?"), registros
        )

        # Armar historial embebido: {"adu-lot": [{fecha_corte, semaforo, freg, fstock, comentario}]}
        lotes_en_reporte = {(r[0], r[1]) for r in registros}
        historial_lotes = {}
        if lotes_en_reporte:
            con_h = sqlite3.connect(HIST_DB); con_h.row_factory = sqlite3.Row
            for (codadu, codlot) in lotes_en_reporte:
                rows_h = [dict(r) for r in con_h.execute("""
                    SELECT p.fecha_corte, r.semaforo, r.comentario, r.freg, r.fstock
                ...
                if rows_h:
                    historial_lotes[f"{codadu}-{codlot}"] = rows_h
            con_h.close()

        html_out  = mod.generar_html(registros, fecha_yymmdd, serie, historial_lotes)
...
def api_stock_download(job_id):
    ...
    # 2. Regenerar desde BD si el proceso se reinició
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    ...
    return jsonify({"ok": False,
                    "error": "HTML expirado de memoria. Regenerá el reporte para descargar."}), 410
