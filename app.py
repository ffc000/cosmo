...
def _procesar_csv(tmp_path, tabla, log):
    ...
    try:
        if tabla.startswith("DAT_"):
            ...
            log.append(f"  {inserted:,} filas insertadas. Calculando fechas ISO...")
...
# ══════════════════════════════════════════════════════════════════════════════
# SUBMÓDULOS CONSULTA DAT / RECHAZOS
# ══════════════════════════════════════════════════════════════════════════════
...
def sintia_dat_query():
    ...
    if p.get("vehiculo"):
        conditions.append("(TRACTOR LIKE ? OR SEMI LIKE ?)");         params += [f"%{p['vehiculo']}%", f"%{p['vehiculo']}%"]
    ...
    anio  = p.get("anio", _dt.date.today().year)
    tabla = f"DAT_{anio}"
    try:
        ...
        if not cur.fetchone():
            con.close(); return jsonify({"ok": False, "error": f"Tabla {tabla} no encontrada en la BD."})
        cur.execute(f"SELECT * FROM {tabla} {where} LIMIT 501", params)
...
def sintia_rec_query():
    ...
    try:
        con = sqlite3.connect(DB_PATH); con.row_factory = sqlite3.Row; cur = con.cursor()
        cur.execute(f"SELECT PaisEmisor, Anio, NroMic, Fecha_ISO, Mes, Metodo, Mensaje FROM RECHAZOS {where} ORDER BY Fecha_ISO DESC LIMIT 501", params)
        rows_raw = cur.fetchall(); con.close()
        cols = list(rows_raw[0].keys()) if rows_raw else []
        truncated = len(rows_raw) > 500
        rows = [list(r) for r in rows_raw[:500]]
...
def sintia_dat_export():
    data = request.json or {}
    buf = _exportar_xlsx(data.get("cols", []), data.get("rows", []))
    return send_file(buf, as_attachment=True, download_name="DAT_consulta.xlsx",
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
def sintia_rec_export():
    data = request.json or {}
    buf = _exportar_xlsx(data.get("cols", []), data.get("rows", []))
    return send_file(buf, as_attachment=True, download_name="RECHAZOS_consulta.xlsx",
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
