            ...
            with open(tmp_path, encoding="utf-8", errors="replace") as fh:
                for raw_line in fh:
                    ...
                    if headers is None:
                        ...
                        # Normalizar nombres de columnas problemáticos
                        col_normalize = {
                            "TIENE NOVEDAD?": "tiene_novedad",
                            "TIENE NOVEDAD": "tiene_novedad",
                        }
...
def sintia_dashboard():
    ...
    try:
        ...
        # Total MICs últimos 30 días vs 30 días anteriores
        ...
        # Por estado (últimos 30 días)
        por_estado = {}
        for est in ["Oficializado", "Cancelado", "Cerrado"]:
            cur.execute(f"SELECT COUNT(*) FROM {tabla} WHERE FECHA_INGRESO_ISO BETWEEN ? AND ? AND EST_MIC = ?", (d30, hoy_iso, est))
            por_estado[est] = cur.fetchone()[0]

        # Rechazos últimos 30 días + alerta de tendencia
        rechazos_mes = 0
        rechazos_7d = 0
        ...
        # Última fecha de ingreso en BD (para detectar desactualización)
...
def sintia_dat_query():
    ...
    if p.get("novedad"):
        for col_nov in ['tiene_novedad', '"TIENE NOVEDAD?"']:
            try:
                con2 = sqlite3.connect(DB_PATH)
                con2.execute(f"SELECT {col_nov} FROM {tabla} LIMIT 1")
                con2.close()
                conditions.append(f"{col_nov} = ?"); params.append(p["novedad"])
                break
            except: continue
    ...
    try:
        ...
        # Resumen sobre el universo completo (sin LIMIT)
        ...
        try:
            ...
            for nombre, cod in paises.items():
                cur.execute(f"SELECT COUNT(*) FROM {tabla} {where} AND MIC LIKE ?", params + [f"%{cod}%"])
                cnt = cur.fetchone()[0]
                if cnt: por_pais[nombre] = cnt
            resumen["por_pais"] = por_pais
...
def sintia_rec_query():
    ...
    try:
        ...
        # Resumen sobre el universo completo
