...
def run_job(job_id, pais, anio, mes_d, mes_h, usar_ia, username, quiere_word=True, quiere_excel=True, quiere_pdf=False):
    ...
    def _log_fn(msg):
        log.append(msg)
        # Fase 6: las anomalías de correr_queries (transmisiones > ingresos
        # del mes) antes solo quedaban en el log del informe, donde nadie
        # las veía hasta abrir el Word. Se reenvían también por Telegram.
        if "Anomalía" in msg:
