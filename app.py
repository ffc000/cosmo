...
def _procesar_csv(tmp_path, tabla, log):
    ...
    try:
        if tabla.startswith("DAT_"):
            ...
            # Streaming: primera pasada para obtener headers
            headers = None
            with open(tmp_path, encoding="utf-8", errors="replace") as f:
                for line in f:
                    s = line.strip()
                    if s and ";" in s and not s.startswith("---") and not (s.upper().startswith("REGISTRO") and ";" not in s):
                        headers = [h.strip() for h in s.split(";")]
                        break
            if not headers:
            ...
            with open(tmp_path, encoding="utf-8", errors="replace") as f:
                for line in f:
                    s = line.strip()
                    if not s or ";" not in s or s.startswith("---") or (s.upper().startswith("REGISTRO") and ";" not in s):
                        continue
                    if first:
                        first = False
            ...
            if batch:
                ...
                inserted += len(batch)
            con.commit()
            log.append(f"Columnas: {len(headers)} | Filas insertadas: {inserted:,}")
            ...
            idx_cols = None; cols_finales = None; placeholders = None; cols_str = None
            inserted = 0; batch = []; batch_size = 5000; first = True
            with open(tmp_path, encoding="utf-8", errors="replace") as f:
                for l in f:
                    s = l.strip()
                    ...
                    if len(batch) >= batch_size:
                        try: cur.executemany(f"INSERT INTO RECHAZOS ({cols_str}) VALUES ({placeholders})", batch); inserted += len(batch)
                            ...
                            for r in batch:
                                try: cur.execute(f"INSERT INTO RECHAZOS ({cols_str}) VALUES ({placeholders})", r); inserted += 1
                                except: pass
                        batch = []; con.commit()
...
@app.route("/api/admin/prompts/<pid>", methods=["GET"])
@login_required
...
@app.route("/api/admin/prompts/<pid>", methods=["PUT"])
@login_required
