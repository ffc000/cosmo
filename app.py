...
def vua_informe():
    ...
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    try:
        config     = [dict(r) for r in con.execute("SELECT * FROM vua_config").fetchall()]
        ejes       = [dict(r) for r in con.execute("SELECT * FROM vua_ejes ORDER BY orden ASC").fetchall()]
        equipo     = [dict(r) for r in con.execute("SELECT * FROM vua_equipo ORDER BY organismo, nombre ASC").fetchall()]
        cronologia = [dict(r) for r in con.execute("SELECT * FROM vua_cronologia ORDER BY orden ASC, id ASC").fetchall()]
        glosario   = [dict(r) for r in con.execute("SELECT * FROM vua_glosario ORDER BY termino ASC").fetchall()]
        riesgos    = [dict(r) for r in con.execute("SELECT * FROM vua_riesgos WHERE activo=1 ORDER BY orden ASC").fetchall()]
        minutas    = [dict(r) for r in con.execute("SELECT * FROM vua_minutas ORDER BY id DESC LIMIT 20").fetchall()]
    finally:
        con.close()
