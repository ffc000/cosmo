    ...
    try:
        ...
        def _contar_u15(arr_bits):
            ...
            for i, bit in enumerate(arr_bits):
                fecha_int = int('20' + arr15_bk[i]) if False else int(arr15_bk[i])  # yymmdd int
        ...
        for r in registros:
            ...
            ft, fnt, fnh   = _contar_u15(u15)
            st, snt, snh   = _contar_u15(u15s)
            tend_data[lot_id] = {
                'freg_t': ft, 'freg_nt': fnt, 'freg_nh': fnh,
                'fstock_t': st, 'fstock_nt': snt, 'fstock_nh': snh,
            }
            tend_batch.append((lot_id, reporte_id, ft, fnt, fnh, st, snt, snh))
        ...
        con_t.executemany("""INSERT OR REPLACE INTO stock_tendencia
            (id, reporte_id, freg_transmitio, freg_no_transmitio, freg_no_habil,
             fstock_transmitio, fstock_no_transmitio, fstock_no_habil)
            VALUES (?,?,?,?,?,?,?,?)""", tend_batch)
        con_t.commit()
        ...
        if lot_ids:
            ...
            rows_prev = con_t.execute(f"""
                SELECT t.id, t.freg_transmitio, t.fstock_transmitio
            ...
            for row in rows_prev:
                lid = row[0]
                if lid not in seen:
                    tend_prev[lid] = {'freg_t': row[1], 'fstock_t': row[2]}
                    seen.add(lid)
        con_t.close()
        ...
        for lot_id, td in tend_data.items():
            ...
            tendencia[lot_id] = {
                'treg':    _tend_symbol(td['freg_t'],   prev['freg_t']   if prev else None),
                'tstock':  _tend_symbol(td['fstock_t'], prev['fstock_t'] if prev else None),
