...
def api_stock_generar():
    import hashlib, importlib.util, os as _os
    ...
    try:
        ...
        _stock_jobs[job_id] = {"html": html_out, "fecha": fecha_dt.strftime("%y%m%d")}
...
def api_stock_download(job_id):
    ...
    html  = job["html"] if isinstance(job, dict) else job
    fecha = job["fecha"] if isinstance(job, dict) else job_id
