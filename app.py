...
def sintia_dashboard():
    ...
    try:
        ...
        # Con novedad (mes actual) — nombre de columna puede variar
        con_novedad = 0
        for col_novedad in ['tiene_novedad', '"TIENE NOVEDAD?"', 'TIENE_NOVEDAD']:
            try:
                cur.execute(f"SELECT COUNT(*) FROM {tabla} WHERE FECHA_INGRESO_ISO LIKE ? AND {col_novedad} = 'SI'", (f"{mes_actual}%",))
...
def sintia_dat_query():
    ...
    try:
        ...
        try:
            ...
            cur.execute(f"SELECT MIC, COUNT(*) as cnt FROM {tabla} {where} GROUP BY MIC ORDER BY cnt DESC LIMIT 10", params)
            resumen["por_mic"] = {r[0]: r[1] for r in cur.fetchall()}
