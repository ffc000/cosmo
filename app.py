        ...
        if tabla.startswith("DAT_"):
            ...
            with open(tmp_path, encoding="utf-8", errors="replace") as fh:
                for raw_line in fh:
                    ...
                    if len(batch) >= batch_size:
                        for r in batch:
                            try: cur.execute(f"INSERT OR IGNORE INTO RECHAZOS ({', '.join(cols_finales)}) VALUES ({placeholders})", r); inserted += cur.rowcount
                            except: skipped += 1
                        con.commit()
                        skipped += len(batch) - inserted - skipped  # aprox
                        batch = []
