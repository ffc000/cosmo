...
def api_training_calendario():
    ...
    for anio, mes in meses:
        try:
            ...
            logging.info(f"CALENDARIO RAW | {anio}-{mes:02d} | type={type(data).__name__} | keys={list(data.keys()) if isinstance(data, dict) else 'list'} | len={len(data) if isinstance(data, (list,dict)) else '?'}")
            if isinstance(data, list) and data:
                logging.info(f"CALENDARIO SAMPLE | {anio}-{mes:02d} | item[0]={data[0]}")
            elif isinstance(data, dict):
                logging.info(f"CALENDARIO SAMPLE | {anio}-{mes:02d} | dict={str(data)[:500]}")
    ...
    # Deduplicar por id (solo cuando existe — Garmin puede devolver el mismo workout en meses solapados)
    seen_ids = set()
    workouts_uniq = []
    for w in workouts:
        wid = w["id"]
        if wid:
            if wid in seen_ids:
                continue
            seen_ids.add(wid)
        workouts_uniq.append(w)
    workouts = workouts_uniq
