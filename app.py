...
# ── Import SQL tabla RECHAZOS ──────────────────────────────────────────────────
@app.route("/api/import-rechazos-sql", methods=["POST"])
@login_required
@admin_required
def import_rechazos_sql():
    """Restaura la tabla RECHAZOS desde un .sql — reemplaza completamente la tabla."""
    if "file" not in request.files:
        return jsonify({"ok": False, "error": "No se recibió archivo"})
    f = request.files["file"]
    if not f.filename.endswith(".sql"):
        return jsonify({"ok": False, "error": "El archivo debe ser .sql"})
    if not os.path.exists(DB_PATH):
        return jsonify({"ok": False, "error": "La BD pad.db no está cargada"})
    tmp_sql = "/tmp/rechazos_import.sql"
    f.save(tmp_sql)
    try:
        con = sqlite3.connect(DB_PATH)
        # Borrar tabla existente y recrear desde el SQL
        con.execute("DROP TABLE IF EXISTS RECHAZOS")
        con.commit(); con.close()
        with open(tmp_sql, "r", encoding="utf-8", errors="replace") as sql_fh:
        ...
        if result.returncode != 0 and result.stderr:
            return jsonify({"ok": False, "error": result.stderr[:500]})
        # Verificar que quedó la tabla y contar registros
        con = sqlite3.connect(DB_PATH)
        try:
            count = con.execute("SELECT COUNT(*) FROM RECHAZOS").fetchone()[0]
        except:
            count = 0
        # Recrear índices si no existen
        try:
            con.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_rechazos_unique ON RECHAZOS(PaisEmisor, NroMic, Fecha, Metodo)")
            con.execute("CREATE INDEX IF NOT EXISTS idx_rechazos_fecha ON RECHAZOS(Fecha_ISO)")
        except: pass
        con.commit(); con.close()
        logging.info(f"RECHAZOS SQL IMPORT | user={session.get('username')} | registros={count:,}")
        return jsonify({"ok": True, "size_gb": round(os.path.getsize(DB_PATH)/(1024**3), 2), "registros": count})
    except subprocess.TimeoutExpired:
        return jsonify({"ok": False, "error": "Timeout — archivo muy grande."})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)})
        ...
        try:
            if os.path.exists(tmp_sql): os.remove(tmp_sql)
        except: pass
...
def _procesar_csv(tmp_path, tabla, log):
    ...
    try:
        if tabla.startswith("DAT_"):
            ...
            log.append("Procesando archivo RECHAZOS (acumulativo, streaming)...")
            ...
            if not cur.fetchone():
                ...
                cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_rechazos_unique ON RECHAZOS(PaisEmisor, NroMic, Fecha, Metodo)")
            else:
                # Solo agregar columnas faltantes — NO borrar datos existentes
                for col in ["Fecha_ISO","Mes","Anio"]:
                    try: cur.execute(f"ALTER TABLE RECHAZOS ADD COLUMN {col} TEXT")
                    except: pass
                try: cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_rechazos_unique ON RECHAZOS(PaisEmisor, NroMic, Fecha, Metodo)")
                except: pass
            con.commit()
            ...
            with open(tmp_path, encoding="utf-8", errors="replace") as fh:
                for raw_line in fh:
                    ...
                    if len(batch) >= batch_size:
                        for r in batch:
                            try:
                                cur.execute(f"INSERT OR IGNORE INTO RECHAZOS ({', '.join(cols_finales)}) VALUES ({placeholders})", r)
                                if cur.rowcount == 1: inserted += 1
                                else: skipped += 1
                            except: skipped += 1
                        con.commit()
                        batch = []
                        log.append(f"  {inserted:,} filas nuevas insertadas...")
            ...
            log.append(f"Filas nuevas: {inserted:,} | Duplicados ignorados: {skipped:,}")
            log.append("Calculando fechas ISO en registros sin fecha calculada...")
            cur.execute("""UPDATE RECHAZOS SET
                ...
                Mes = CASE CAST(SUBSTR(CAST(Fecha AS TEXT),LENGTH(CAST(Fecha AS TEXT))-5,2) AS INTEGER)
                    ...
                    END
                WHERE Fecha IS NOT NULL AND LENGTH(CAST(Fecha AS TEXT))>=6 AND Fecha_ISO IS NULL""")
            try: cur.execute("CREATE INDEX IF NOT EXISTS idx_rechazos_fecha ON RECHAZOS(Fecha_ISO)")
            except: pass
            con.commit(); con.close()
            log.append(f"✓ RECHAZOS: {inserted:,} registros nuevos acumulados, fechas calculadas")
        else:
            con.close()
...
def _get_fernet():
    """
    Devuelve una instancia de Fernet usando SECRET_KEY como base.
    Si cryptography no está disponible, cae a XOR como fallback con advertencia.
    """
    try:
...
def _encrypt(text: str) -> str:
    """Cifra texto con Fernet (AES-128-CBC + HMAC). Fallback a XOR si cryptography no está instalado."""
    if not text:
        return text
    f = _get_fernet()
    if f:
        return f.encrypt(text.encode()).decode()
    # Fallback XOR (deprecado — instalar cryptography)
    logging.warning("GARMIN CREDS | cryptography no instalado, usando XOR. Ejecutar: pip install cryptography")
    key_bytes = ("cosmotools2026" * (len(text) // 14 + 1)).encode()
    return ''.join(chr(ord(c) ^ key_bytes[i]) for i, c in enumerate(text))

def _decrypt(text: str) -> str:
    """Descifra texto con Fernet. Detecta automáticamente si el valor está en XOR legacy."""
    if not text:
        return text
    f = _get_fernet()
    if f:
        try:
            ...
            # Valor legacy en XOR — intentar desencriptar con XOR y re-encriptar con Fernet
            logging.info("GARMIN CREDS | detectado valor legacy XOR, migrando a Fernet")
            key_bytes = ("cosmotools2026" * (len(text) // 14 + 1)).encode()
            return ''.join(chr(ord(c) ^ key_bytes[i]) for i, c in enumerate(text))
    # Sin cryptography — XOR directo
    key_bytes = ("cosmotools2026" * (len(text) // 14 + 1)).encode()
    return ''.join(chr(ord(c) ^ key_bytes[i]) for i, c in enumerate(text))
...
def get_credenciales_garmin():
    try:
        ...
        con.close()
        usuario = _decrypt(u["valor"]) if u and u["valor"] else ""
        passwd  = _decrypt(p["valor"]) if p and p["valor"] else ""
...
def set_credenciales_garmin(usuario: str, passwd: str):
    ...
    con.execute("INSERT OR REPLACE INTO garmin_config (clave,valor,modificado) VALUES (?,?,datetime('now'))",
                ("garmin_user", _encrypt(usuario)))
    con.execute("INSERT OR REPLACE INTO garmin_config (clave,valor,modificado) VALUES (?,?,datetime('now'))",
                ("garmin_pass", _encrypt(passwd)))
    con.commit(); con.close()
...
BACKUP_FILES = {
    "historial": os.path.join(BACKUP_DIR, "historial_backup.db"),
    "pad":       os.path.join(BACKUP_DIR, "pad_backup.db"),
}
...
def _hacer_backup():
    ...
    resultados = {}
    try:
        src = _sq3.connect(HIST_DB)
        dst = _sq3.connect(BACKUP_FILES["historial"])
        src.backup(dst)
        src.close(); dst.close()
        size = round(os.path.getsize(BACKUP_FILES["historial"]) / (1024**2), 1)
        resultados["historial"] = {"ok": True, "size_mb": size}
        logging.info(f"BACKUP | historial.db -> {size} MB")
    except Exception as e:
        resultados["historial"] = {"ok": False, "error": str(e)}
        logging.error(f"BACKUP ERROR historial | {e}")
    if os.path.exists(DB_PATH):
        try:
            src = _sq3.connect(DB_PATH)
            dst = _sq3.connect(BACKUP_FILES["pad"])
            src.backup(dst)
            src.close(); dst.close()
            size = round(os.path.getsize(BACKUP_FILES["pad"]) / (1024**2), 1)
            resultados["pad"] = {"ok": True, "size_mb": size}
            logging.info(f"BACKUP | pad.db -> {size} MB")
        except Exception as e:
            resultados["pad"] = {"ok": False, "error": str(e)}
            logging.error(f"BACKUP ERROR pad | {e}")
    else:
        resultados["pad"] = {"ok": False, "error": "pad.db no existe"}
    try:
        con = sqlite3.connect(HIST_DB)
        con.execute("CREATE TABLE IF NOT EXISTS backup_log (id INTEGER PRIMARY KEY AUTOINCREMENT, fecha TEXT, resultados TEXT)")
        con.execute("INSERT INTO backup_log (fecha, resultados) VALUES (datetime('now'), ?)", (json.dumps(resultados),))
        con.commit(); con.close()
    except: pass
    return resultados
...
def _scheduler_backup():
    ...
    while True:
        ...
        if dias_hasta_domingo == 0 and ahora.hour >= 2:
            dias_hasta_domingo = 7
        proximo = ahora.replace(hour=2, minute=0, second=0, microsecond=0) + timedelta(days=dias_hasta_domingo)
        espera = (proximo - ahora).total_seconds()
        logging.info(f"BACKUP SCHEDULER | proximo backup en {espera/3600:.1f}h ({proximo.strftime('%Y-%m-%d %H:%M')})")
        time.sleep(espera)
        _hacer_backup()

_threading.Thread(target=_scheduler_backup, daemon=True).start()
...
def backup_estado():
    ...
    try:
        con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
        row = con.execute("SELECT fecha, resultados FROM backup_log ORDER BY id DESC LIMIT 1").fetchone()
        con.close()
        if row:
            ultimo = {"fecha": row["fecha"], "resultados": json.loads(row["resultados"])}
    except: pass
    archivos = {}
    for nombre, path in BACKUP_FILES.items():
    ...
    if dias_hasta_domingo == 0 and ahora.hour >= 2:
        dias_hasta_domingo = 7
    proximo = ahora.replace(hour=2, minute=0, second=0, microsecond=0) + timedelta(days=dias_hasta_domingo)
    return jsonify({"ok": True, "ultimo": ultimo, "archivos": archivos, "proximo": proximo.strftime("%Y-%m-%d %H:%M")})

@app.route("/api/admin/backup/ejecutar", methods=["POST"])
@login_required
@admin_required
def backup_ejecutar():
    logging.info(f"BACKUP MANUAL | user={session.get('username')}")
    resultados = _hacer_backup()
    ok_total = all(v.get("ok") for v in resultados.values())
    return jsonify({"ok": ok_total, "resultados": resultados})

@app.route("/api/admin/backup/download/<nombre>")
@login_required
@admin_required
def backup_download(nombre):
    if nombre not in BACKUP_FILES:
        return jsonify({"ok": False, "error": "Nombre invalido"}), 400
    path = BACKUP_FILES[nombre]
    if not os.path.exists(path):
        return jsonify({"ok": False, "error": "Backup no disponible"}), 404
    return send_file(path, as_attachment=True, download_name=os.path.basename(path))
