...
def registrar_sesion(username, token, ip, ua):
    con = sqlite3.connect(HIST_DB)
    con.execute("INSERT OR REPLACE INTO sesiones (token,username,ip,user_agent,creado,ultimo_acceso,activa) VALUES (?,?,?,?,datetime('now'),datetime('now'),1)",
        (token, username, ip, ua))
    con.commit(); con.close()
...
def revocar_sesion(token):
    con = sqlite3.connect(HIST_DB)
    con.execute("UPDATE sesiones SET activa=0 WHERE token=?", (token,))
    con.execute("INSERT OR IGNORE INTO tokens_revocados (token) VALUES (?)", (token,))
    con.commit(); con.close()
...
def get_prompt(prompt_id):
    con = sqlite3.connect(HIST_DB)
    row = con.execute("SELECT contenido FROM prompts WHERE id=?", (prompt_id,)).fetchone()
    con.close()
    return row[0] if row else None

# ── Login con BD de usuarios ──────────────────────────────────────────────────
# Reemplazar login original con sistema de tokens
# ── API Usuarios ──────────────────────────────────────────────────────────────
def admin_usuarios_list():
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    rows = [dict(r) for r in con.execute(
        "SELECT id, username, rol, modulos, activo, creado, ultimo_acceso FROM usuarios ORDER BY creado DESC"
    ).fetchall()]
    con.close()
...
def admin_usuarios_create():
    ...
    try:
        ...
        con.execute("INSERT INTO usuarios (username, password_hash, rol, modulos) VALUES (?,?,?,?)",
            (username, ph, rol, modulos))
        con.commit(); con.close()
        logging.info(f"USUARIO CREADO | admin={session.get('username')} | nuevo={username} | rol={rol}")
...
def admin_usuarios_update(uid):
    ...
    if row and row[0] == session.get("username"):
        con.close()
        return jsonify({"ok": False, "error": "No podés editar tu propio usuario desde acá"})
    fields = []
    params = []
    if "rol" in data: fields.append("rol=?"); params.append(data["rol"])
    if "modulos" in data: fields.append("modulos=?"); params.append(data["modulos"])
    if "activo" in data: fields.append("activo=?"); params.append(int(data["activo"]))
    ...
    params.append(uid)
    con.execute(f"UPDATE usuarios SET {', '.join(fields)} WHERE id=?", params)
    con.commit(); con.close()
    logging.info(f"USUARIO EDITADO | admin={session.get('username')} | uid={uid}")
    return jsonify({"ok": True})
def admin_usuarios_delete(uid):
    ...
    if row[0] == session.get("username"):
        con.close()
        return jsonify({"ok": False, "error": "No podés eliminar tu propio usuario"})
    con.execute("UPDATE usuarios SET activo=0 WHERE id=?", (uid,))
    con.commit(); con.close()
    logging.info(f"USUARIO DESACTIVADO | admin={session.get('username')} | uid={uid}")
    return jsonify({"ok": True})
...
def admin_sesiones_list():
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    rows = [dict(r) for r in con.execute("""
        SELECT token, username, ip, user_agent, creado, ultimo_acceso
        FROM sesiones WHERE activa=1
        AND ultimo_acceso > datetime('now', '-4 hours')
        ORDER BY ultimo_acceso DESC
    """).fetchall()]
    con.close()
    # Marcar la sesión propia
    mi_token = session.get("token","")
    for r in rows:
        r["es_propia"] = (r["token"] == mi_token)
        r["token"] = r["token"][:8] + "..."  # Ocultar token completo
    return jsonify({"ok": True, "rows": rows})
def admin_sesiones_revocar(token_prefix):
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    # Buscar token completo por prefijo
    rows = con.execute("SELECT token, username FROM sesiones WHERE token LIKE ? AND activa=1",
        (token_prefix + "%",)).fetchall()
    con.close()
    if not rows:
        return jsonify({"ok": False, "error": "Sesión no encontrada"})
    for row in rows:
        if row["username"] == session.get("username"):
            return jsonify({"ok": False, "error": "No podés cerrar tu propia sesión desde acá"})
        revocar_sesion(row["token"])
        logging.info(f"SESION REVOCADA | admin={session.get('username')} | usuario={row['username']}")
    return jsonify({"ok": True})
def admin_sesiones_revocar_todas():
    mi_token = session.get("token","")
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    rows = con.execute("SELECT token FROM sesiones WHERE activa=1 AND token != ?", (mi_token,)).fetchall()
    con.close()
    for row in rows:
        revocar_sesion(row["token"])
    logging.info(f"TODAS LAS SESIONES REVOCADAS | admin={session.get('username')}")
...
def admin_prompts_list():
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    rows = [dict(r) for r in con.execute(
        "SELECT id, nombre, descripcion, modulo, modificado FROM prompts ORDER BY modulo, nombre"
    ).fetchall()]
    con.close()
    return jsonify({"ok": True, "rows": rows})
def admin_prompts_get(prompt_id):
    con = sqlite3.connect(HIST_DB); con.row_factory = sqlite3.Row
    row = con.execute("SELECT * FROM prompts WHERE id=?", (prompt_id,)).fetchone()
    con.close()
    if not row: return jsonify({"ok": False, "error": "Prompt no encontrado"})
    return jsonify({"ok": True, "prompt": dict(row)})
def admin_prompts_update(prompt_id):
    data = request.json or {}
    contenido = data.get("contenido","").strip()
    if not contenido:
        return jsonify({"ok": False, "error": "El contenido no puede estar vacío"})
    con = sqlite3.connect(HIST_DB)
    con.execute("UPDATE prompts SET contenido=?, modificado=datetime('now') WHERE id=?",
        (contenido, prompt_id))
    con.commit(); con.close()
    logging.info(f"PROMPT EDITADO | admin={session.get('username')} | id={prompt_id}")
    return jsonify({"ok": True})
def admin_prompts_reset(prompt_id):
    # Restaurar desde generar.py / app.py leyendo el prompt original
    return jsonify({"ok": False, "error": "Reset manual: editá el prompt desde la interfaz"})
...
def vua_minuta_ia():
    ...
    try:
        ...
        t_txt = chr(10).join(["- " + t for t in temas])
        prompt = ("Sos analista de DI REPA. Genera borrador de acta VUA."
            + chr(10) + "ASUNTO: " + asunto
            + chr(10) + "PARTICIPANTES: " + p_txt
            + chr(10) + "TEMAS:" + chr(10) + t_txt
            + chr(10) + chr(10) + "Devuelve SOLO JSON: {\"temas_tratados\":[\"p1\"],\"acuerdos\":[\"a1\"],\"proximos_pasos\":[\"p1\"]}"
            + chr(10) + "Formal, tecnico, espanol rioplatense.")
        msg = client.messages.create(model="claude-haiku-4-5-20251001", max_tokens=1500,
    ...
    if request.method == "POST":
        ...
        if autenticado:
            ...
            session.update({
                ...
                "token": token, "last_active": datetime.now().timestamp(),
...
def admin_index():
    ...
    return render_template("admin.html",
        db_exists=db_exists, db_size=db_size, now=hoy,
        role=session.get("role","admin"), username=session.get("username",""),
        pendientes=pendientes)

# ══════════════════════════════════════════════════════════════════════════════
# GESTIÓN DE USUARIOS, SESIONES Y PROMPTS
# ══════════════════════════════════════════════════════════════════════════════
import secrets
...
def login2():
    ...
    if request.method == "POST":
        ...
        if user and check_password(p, user["password_hash"]):
            ...
            session["username"] = u
            session["token"] = token
            session["modulos"] = user["modulos"].split(",")
            session["last_active"] = datetime.now().timestamp()
            session.permanent = True
            registrar_sesion(u, token, ip, ua)
...
@app.route("/api/admin/prompts/<prompt_id>/reset", methods=["POST"])
@login_required
@admin_required
def admin_prompts_reset(prompt_id):
    # Restaurar desde generar.py / app.py leyendo el prompt original
