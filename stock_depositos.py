...
.modal-box{{background:#fff;border-radius:10px;padding:1.5rem;width:80vw;max-height:90vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,.18);position:relative}}
...
.modal-15{{overflow-x:auto;margin-bottom:.5rem;-webkit-overflow-scrolling:touch}}
.modal-15 table{{font-size:.7rem;border-collapse:collapse;table-layout:fixed;white-space:nowrap}}
.modal-15 th{{background:#1E2A3B;color:#fff;padding:.4rem .4rem;text-align:center;font-size:.63rem;min-width:52px}}
.modal-15 td{{padding:.4rem .4rem;text-align:center;border-bottom:1px solid #f0f0f0;font-size:.63rem;min-width:52px}}
...
<div class="modal-overlay" id="modal-grafico-overlay" onclick="if(event.target===this)cerrarGrafico()">
  <div style="background:#fff;border-radius:10px;padding:1.5rem;max-width:860px;width:96vw;box-shadow:0 8px 32px rgba(0,0,0,.18);position:relative">
...
function abrirModal(idx) {{
  ...
  document.getElementById('modal-15').innerHTML =
    '<table style="border-collapse:collapse;width:100%"><thead>' + thead + '</thead><tbody>' + fregRow + fstockRow + '</tbody></table>';
