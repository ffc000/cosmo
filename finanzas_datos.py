...
def get_tarjetas(db_path: str):
    with get_db(db_path, row_factory=True) as con:
        return [dict(r) for r in con.execute("SELECT * FROM fin_tarjetas WHERE activa=1").fetchall()]
...
def buscar_posible_duplicado(db_path: str, tarjeta_id: str, fecha: str, monto: float, dias: int = 5):
    """Busca un movimiento existente con el mismo monto (parte entera, sin
    decimales) dentro de un rango de días, para evitar cargar dos veces el
    mismo gasto entre carga manual y resumen de tarjeta.

    No es un bloqueo automático: devuelve las coincidencias para que quien
    llama decida (avisar al usuario, no descartar solo, ya que dos gastos
    genuinos del mismo monto y fecha son posibles).
    """
    if not monto:
        return []
    f = datetime.strptime(fecha, "%Y-%m-%d")
    desde = (f - timedelta(days=dias)).strftime("%Y-%m-%d")
    hasta = (f + timedelta(days=dias)).strftime("%Y-%m-%d")
    monto_entero = int(monto)
    with get_db(db_path, row_factory=True) as con:
        rows = con.execute("""
            SELECT id, fecha, descripcion, monto_ars, origen
            FROM fin_movimientos
            WHERE tarjeta_id=? AND fecha BETWEEN ? AND ?
              AND monto_ars != 0
              AND CAST(monto_ars AS INTEGER) = ?
        """, (tarjeta_id, desde, hasta, monto_entero)).fetchall()
...
def listar_ddjj(db_path: str):
    with get_db(db_path, row_factory=True) as con:
        return [dict(r) for r in con.execute("SELECT * FROM fin_ddjj ORDER BY anio DESC").fetchall()]
