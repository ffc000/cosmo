...
function buildResumenEjecutivo(config) {
...
function buildAntecedentes(config) {
...
function buildEjes(ejes) {
  const items = [];
  items.push(heading("3. Ejes de Trabajo"));
  items.push(divider());
  ...
  const resumenRows = ejes.map(eje => new TableRow({
    children: [
      ...
      tc(eje.nombre, 5500),
      tc(eje.estado, 2871, { fill: estadoBadge(eje.estado), align: AlignmentType.CENTER }),
    ]
  }));
  ...
  ejes.forEach((eje, idx) => {
    items.push(subheading(`${eje.id}. ${eje.nombre}`));

    if (eje.descripcion) {
      items.push(p(eje.descripcion, { after: 120 }));
    }
      ...
      bloques.push(new TableRow({
        children: [
          tcMulti([
            ...
            new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: eje.propuesta_vucea, font: "Arial", size: 20, color: C.text })] }),
          ], CW, { fill: "EFF6FF" })
        ]
      }));
    }
      ...
      bloques.push(new TableRow({
        children: [
          tcMulti([
            ...
            new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: eje.postura_aduana, font: "Arial", size: 20, color: C.text })] }),
          ], CW, { fill: C.lightW })
        ]
      }));
    }
      ...
      bloques.push(new TableRow({
        children: [
          tcMulti([
            ...
            new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: eje.recomendacion, font: "Arial", size: 20, color: C.text })] }),
          ], CW, { fill: C.lightG })
        ]
      }));
    }
...
function buildRiesgos(riesgos) {
  const items = [];
  items.push(heading("4. Riesgos Identificados"));
  items.push(divider());
  items.push(empty(120));
  ...
  riesgos.forEach((r, idx) => {
    ...
    items.push(new Table({
      ...
      rows: [
        ...
        new TableRow({
          children: [
            tc(r.codigo || `${idx+1}`, 700, { fill: C.navy, bold: true, color: C.white, size: 20 }),
            tc(r.titulo, 5500, { fill: C.navy, bold: true, color: C.white, size: 20 }),
        ...
        new TableRow({
          children: [
            ...
            tcMulti([new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: r.descripcion || "", font: "Arial", size: 20 })] })], 7771),
          ]
        }),
        // Mitigación
        new TableRow({
          children: [
            ...
            tcMulti([new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: r.mitigacion || "", font: "Arial", size: 20 })] })], 7771, { fill: C.lightG }),
          ]
        }),
      ]
    }));
...
function buildEquipo(equipo) {
  const items = [];
  items.push(heading("5. Equipo del Proyecto"));
  items.push(divider());
  items.push(empty(120));
  ...
  Object.entries(grupos).forEach(([org, miembros]) => {
    ...
    const rows = miembros.map(m => new TableRow({
      children: [
        tc(m.nombre, 2800, { bold: true, color: C.navy }),
        tc(m.cargo, 4000),
...
function buildCronologia(cronologia) {
  const items = [];
  items.push(heading("6. Cronología de Actividades"));
  items.push(divider());
  items.push(empty(120));
...
function buildGlosario(glosario) {
  const items = [];
  items.push(heading("7. Glosario"));
  items.push(divider());
  items.push(empty(120));

  const rows = glosario.map(g => new TableRow({
    children: [
      tc(g.termino, 2200, { bold: true, color: C.navy, fill: C.light }),
      tc(g.definicion, 5600),
...
function buildMinutas(minutas) {
  if (!minutas || minutas.length === 0) return [];
  const items = [];
  items.push(heading("8. Minutas de Reuniones"));
  items.push(divider());
  items.push(empty(120));

  const rows = minutas.map(m => new TableRow({
    children: [
      tc(m.fecha || "", 1300, { size: 19, color: C.gray }),
      tc(m.asunto || "", 4000, { bold: true }),
      tc(m.lugar || "", 1800, { size: 18, color: C.gray }),
      tc(m.participantes || "", 1971, { size: 18, color: C.gray }),
    ]
  }));
...
function buildDocument(datos) {
  ...
  const content = [
    ...
    ...buildResumenEjecutivo(config),
    ...buildAntecedentes(config),
    ...buildEjes(ejes),
    ...buildRiesgos(riesgos),
    ...buildEquipo(equipo),
    ...buildCronologia(cronologia),
    ...buildGlosario(glosario),
    ...buildMinutas(minutas),
  ];
...
function buildMinutaCompleta(datos) {
  ...
  items.push(empty(200));
  items.push(p(
    "Documento generado por CosmoTools — DI REPA / DV MPAD — ARCA",
    { align: AlignmentType.CENTER, color: C.gray, size: 18, italic: true }
  ));
