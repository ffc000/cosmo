...
def correr_queries(ruta_db, pais, anio, mes_d, mes_h, log_fn):
    ...
    mes_ult, _ = ultimo_mes_completo()
    mes_ant = str(int(mes_ult)-1).zfill(2) if int(mes_ult) > 1 else "12"
    per_ult = f"{anio}-{mes_ult}"; per_ant = f"{anio}-{mes_ant}"
    ...
    rechazos_mes = q("""WITH m AS (SELECT NroMic, MIN(strftime('%Y-%m',Fecha_ISO)) AS periodo FROM RECHAZOS WHERE PaisEmisor=? AND Anio=? AND Metodo='OficializarMicDta' AND strftime('%Y-%m',Fecha_ISO) BETWEEN ? AND ? GROUP BY NroMic) SELECT periodo, COUNT(*) AS MIC_RECHAZOS FROM m GROUP BY periodo ORDER BY periodo""", (pais, anio, desde, hasta))
    rechazos_cat = q("""
    ...
    for cat in top_cats:
        ...
        rechazos_ej.extend(q("""SELECT Fecha_ISO, NroMic, Mensaje FROM (SELECT Fecha_ISO, NroMic, Mensaje, ROW_NUMBER() OVER (PARTITION BY NroMic ORDER BY Fecha_ISO DESC) AS rn FROM RECHAZOS WHERE PaisEmisor=? AND Anio=? AND Metodo='OficializarMicDta' AND strftime('%Y-%m',Fecha_ISO) BETWEEN ? AND ?) WHERE rn=1 AND Mensaje LIKE ? ORDER BY Fecha_ISO DESC LIMIT 5""", (pais, anio, desde, hasta, like_cat)))
    def totales_mes(periodo):
        rows = q(f"""SELECT SUM(CASE WHEN FECHA_TRANS_ISO IS NOT NULL AND FECHA_TRANS_ISO<=FECHA_INGRESO_ISO THEN 1 ELSE 0 END) AS trans, SUM(CASE WHEN FECHA_TRANS_ISO IS NULL THEN 1 ELSE 0 END) AS no_trans, SUM(CASE WHEN FECHA_TRANS_ISO IS NOT NULL AND FECHA_TRANS_ISO>FECHA_INGRESO_ISO THEN 1 ELSE 0 END) AS tardio, COUNT(*) AS total FROM {tabla} WHERE MIC LIKE ? AND strftime('%Y-%m',FECHA_INGRESO_ISO)=?""", (like, periodo))
    ...
    """, (pais, anio, per_ult))
    rech_ant = q("""WITH p AS (SELECT NroMic FROM RECHAZOS WHERE PaisEmisor=? AND Anio=? AND Metodo='OficializarMicDta' AND strftime('%Y-%m',Fecha_ISO)=? GROUP BY NroMic) SELECT COUNT(*) AS total FROM p""", (pais, anio, per_ant))
...
- PAD: "Portal Aduanero" (NUNCA "Plataforma Argentina Digital", "Plataforma de Datos Aduaneros" ni otra variante)
...
- Rechazos: {datos['total_rechazos']}
- Cargados trans/no_trans/tardio: {datos['c_trans']}/{datos['c_no_trans']}/{datos['c_tardio']} de {datos['c_total']}
- Lastre trans/no_trans/tardio: {datos['l_trans']}/{datos['l_no_trans']}/{datos['l_tardio']} de {datos['l_total']}
...
Estilo: formal, t\u00e9cnico, espa\u00f1ol rioplatense. Sin markdown, sin t\u00edtulos. No repetir el nombre de la secci\u00f3n al inicio del texto. Evitar anglicismos (usar "desempe\u00f1o" en lugar de "performance", etc.)."""
        msg = client.messages.create(model="claude-haiku-4-5-20251001", max_tokens=800,
            messages=[{"role":"user","content":prompt}])
        result = [p.strip() for p in msg.content[0].text.split("---BLOQUE---") if p.strip()]
        return result
...
Transmisi\u00f3n: {datos['ult_trans']} transmitidos ({datos['ult_pct_trans']}), {datos['ult_no_trans']} no transmitidos ({datos['ult_pct_no_trans']}), {datos['ult_tardio']} tard\u00edos ({datos['ult_pct_tardio']})
  \u2014 Cargados: trans {datos['ult_pct_trans_carg']}, no trans {datos['ult_pct_no_trans_carg']}, tard\u00edo {datos['ult_pct_tardio_carg']}
...
Rechazos: {datos['ant_rechazos']}

\u2550\u2550\u2550 ACUMULADO DEL PER\u00cdODO ({datos['periodo']}) \u2550\u2550\u2550
...
**Volumen operativo**
[P\u00e1rrafo: total operaciones, desglose impo/expo, comparaci\u00f3n con mes anterior]

**Transmisi\u00f3n anticipada \u2014 [calificaci\u00f3n breve]**
[P\u00e1rrafo: an\u00e1lisis detallado % transmisi\u00f3n \u00faltimo mes vs anterior y vs acumulado. Mencionar cargados vs lastre.]

**Rechazos \u2014 [calificaci\u00f3n breve]**
[P\u00e1rrafo: volumen total, separar duplicados de rechazos operativos, analizar categor\u00edas principales]

**Conclusi\u00f3n**
[P\u00e1rrafo final: s\u00edntesis del estado actual, alertas o recomendaciones concretas]

Estilo: formal, t\u00e9cnico, espa\u00f1ol rioplatense. Sin markdown extra, solo los t\u00edtulos en negrita indicados. Las calificaciones en los subt\u00edtulos van en min\u00fasculas, sin may\u00fasculas ni siglas. Evitar anglicismos ("performance" \u2192 "desempe\u00f1o"). Ser directo y concreto."""
...
def _generar_word(pais, anio, mes_d, mes_h, version,
    ...
    pais_nombre=PAISES.get(pais,pais); periodo=periodo_texto(anio,mes_d,mes_h)
    ...
    if conclusion_ia:
        for bloque in conclusion_ia.split("\n\n"):
            bloque=bloque.strip()
        ...
        ult_expo_pct=pct_f(ult_expo_n,ult_t)
        ...
        if diff_trans>=20: cal_trans="mejora significativa"
        elif diff_trans>=5: cal_trans="mejora moderada"
        elif diff_trans>=0: cal_trans="leve mejora"
        elif diff_trans>=-5: cal_trans="leve deterioro"
        else: cal_trans="deterioro"
        rech_ant_n=total_rech_ant
        ...
        bold_par(doc,f"Transmisi\u00f3n anticipada \u2014 {cal_trans}",
            ...
            f"({pct(carg_trans_n,carg_tot_n)} en cargados, {pct(last_trans_n,last_tot_n)} en lastre), "
            ...
            f"Las transmisiones tard\u00edas tambi\u00e9n {'bajaron' if ult_td<ant_td else 'subieron'} al {pct(ult_td,ult_t)} ({fmt(ult_td)} operaciones vs. {fmt(ant_td)} en {mes_label_largo(per_ant)}).")
        ...
        if rult_op>0 and top_rech:
            ...
            concl+=f"Sin embargo, la irrupci\u00f3n de nuevas categor\u00edas de rechazo \u2014especialmente {cat_nueva}\u2014 indica que el crecimiento en el volumen de transmisiones va acompa\u00f1ado de inconsistencias en la calidad de los datos enviados, lo que requiere atenci\u00f3n focalizada."
...
def generar_informe(ruta_db, pais, anio, mes_d, mes_h, usar_ia, api_key, carpeta, log_fn):
    ...
    if usar_ia and api_key and ANT_OK:
        ...
        conclusion_ia=generar_conclusion_ia({
            ...
            "ult_total":fmt(ult_t),"ult_impo":fmt(n(impo_ult.get("total",0))),"ult_expo":fmt(n(expo_ult.get("total",0))),
            ...
            "ult_pct_no_trans_carg":pct(ult_carg_tot-carg_tr-n(impo_ult.get("tardio",0))-n(expo_ult.get("tardio",0)),ult_carg_tot) if ult_carg_tot else "N/D",
            "ult_pct_tardio_carg":pct(n(impo_ult.get("tardio",0))+n(expo_ult.get("tardio",0)),ult_carg_tot) if ult_carg_tot else "N/D",
            "ult_pct_trans_last":pct(last_trans_n_ia,ult_last_tot) if ult_last_tot else "N/D",
            "ult_last_trans_n":fmt(last_trans_n_ia),
            "ult_pct_no_trans_last":pct(ult_last_tot-last_trans_n_ia-n(impo_ult.get("tardio",0))-n(expo_ult.get("tardio",0)),ult_last_tot) if ult_last_tot else "N/D",
            ...
            "ant_rechazos":fmt(total_rech_ant),"ev_tabla_texto":" | ".join(ev_tabla_lineas),
            "g_total":fmt(gTot),"g_trans":fmt(gT),"g_no_trans":fmt(gN),"g_tardio":fmt(gTd),
            "pct_trans":pct(gT,gTot),"pct_no_trans":pct(gN,gTot),"pct_tardio":pct(gTd,gTot),
            "tiene_interanual":datos_interanual is not None,"anio_ant":anio_ant,
            "g_total_ant":fmt(ia_t),"pct_trans_ant":pct(ia_tr,ia_t),
            "pct_no_trans_ant":pct(ia_nt,ia_t),"pct_tardio_ant":pct(ia_td,ia_t),
        }, api_key)
        if conclusion_ia: log_fn("✓ Conclusi\u00f3n generada")
