...
def test_ddjj_tarjetas_ejecuta_sin_nameerror(client, test_user):
    """Regresión: blueprints/finanzas.py usaba app.secret_key (para descifrar
    números de tarjeta) sin importar 'app' desde core — pasaba el smoke test
    (que solo chequea el redirect a /login) pero rompía en producción al
    ejecutar la ruta de verdad ('name app is not defined')."""
    client.post("/login", data={"user": test_user["username"], "pass": test_user["password"]})
    resp = client.get("/api/finanzas/ddjj/no-existe-este-id/tarjetas")
