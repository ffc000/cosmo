...
Transmisi\u00f3n: {datos['ult_trans']} transmitidos ({datos['ult_pct_trans']}), {datos['ult_no_trans']} no transmitidos ({datos['ult_pct_no_trans']}), {datos['ult_tardio']} tard\u00edos ({datos['ult_pct_tardio']})
  ...
  — Lastre último mes: {datos['ult_last_trans_n']} transmitidos ({datos['ult_pct_trans_last']}) de {datos['ult_last_tot_n']} lastre totales. Variación lastre vs mes anterior: {datos['var_last_trans_pp']} pp.
Si ult_last_trans_n es 0, escribí 'ninguna operación lastre fue transmitida' — NUNCA uses frases como 'por redondeo' o '0 movimientos por redondeo'.
Rechazos del mes: total={datos['ult_rechazos']}, duplicados={datos['ult_rech_duplicados']}, operativos reales={datos['ult_rech_operativos']}. Variación vs mes anterior: {datos['var_rech_dir']} de exactamente {datos['var_rech_abs_texto']} ({datos['var_rech_pct']}). Usá EXACTAMENTE este texto — "{datos['var_rech_abs_texto']}" es la diferencia correcta. NO hagas restas: cualquier resta que calcules puede diferir en 1 por redondeo.
...
Rechazos: {datos['ant_rechazos']} (ÚNICO valor válido — usá EXCLUSIVAMENTE este número)
...
def generar_informe(ruta_db, pais, anio, mes_d, mes_h, usar_ia, api_key, carpeta, log_fn):
    ...
    if usar_ia and api_key and ANT_OK:
        ...
        conclusion_ia=generar_conclusion_ia({
            ...
            "var_last_trans_pp":f"{round(pct_f(last_trans_n_ia,ult_last_tot)-pct_f(ant_tr,ant_t),1):+.1f}".replace(".",",") if ult_last_tot and ant_t else "N/D",
            ...
            "var_rech_abs_texto":f"{fmt(abs(ult_rech_total-total_rech_ant))} rechazos",
