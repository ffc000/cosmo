...
Transmisi\u00f3n: {datos['ult_trans']} transmitidos ({datos['ult_pct_trans']}), {datos['ult_no_trans']} no transmitidos ({datos['ult_pct_no_trans']}), {datos['ult_tardio']} tard\u00edos ({datos['ult_pct_tardio']})
  ...
  — Lastre último mes ({datos['mes_ult_nombre']}): {datos['ult_last_trans_n']} transmitidos ({datos['ult_pct_trans_last']}), {datos['ult_last_notrans_n']} NO transmitidos ({datos['ult_pct_no_trans_last']}), {datos['ult_last_tardio_n']} tardíos ({datos['ult_pct_tardio_last']}). Total lastre: {datos['ult_last_tot_n']}.
  — Lastre mes anterior ({datos['mes_ant_nombre']}): {datos['ant_last_trans_n']} transmitidos de {datos['ant_last_tot_n']} total = {datos['ant_last_pct_trans']} de transmisión.
  — Variación lastre (lastre_actual% vs lastre_anterior%): {datos['var_last_trans_pp']} pp. CRÍTICO: esta variación compara LASTRE vs LASTRE, nunca uses la tasa total del mes anterior para calcularla.
REGLA ABSOLUTA: el número de lastre NO transmitidos es {datos['ult_last_notrans_n']} — NUNCA uses el total de lastre ({datos['ult_last_tot_n']}) como si fuera los no transmitidos. Los no transmitidos = total - transmitidos - tardíos.
