...
Rechazos del mes: total={datos['ult_rechazos']}, duplicados={datos['ult_rech_duplicados']}, operativos reales={datos['ult_rech_operativos']}. Variación vs mes anterior: {datos['var_rech_dir']} de {datos['var_rech_abs']} ({datos['var_rech_pct']}). Usá EXACTAMENTE estos valores pre-calculados. NO calcules restas ni variaciones por tu cuenta — cualquier diferencia que calcules puede tener error de redondeo.
...
def generar_informe(ruta_db, pais, anio, mes_d, mes_h, usar_ia, api_key, carpeta, log_fn):
    ...
    if usar_ia and api_key and ANT_OK:
        ...
        conclusion_ia=generar_conclusion_ia({
            ...
            "var_rech_abs_num":abs(ult_rech_total-total_rech_ant),
