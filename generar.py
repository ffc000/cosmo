...
Transmisi\u00f3n: {datos['ult_trans']} transmitidos ({datos['ult_pct_trans']}), {datos['ult_no_trans']} no transmitidos ({datos['ult_pct_no_trans']}), {datos['ult_tardio']} tard\u00edos ({datos['ult_pct_tardio']})
  ...
  — Lastre último mes: {datos['ult_last_trans_n']} transmitidos ({datos['ult_pct_trans_last']}), {datos['ult_last_tot_n']} - {datos['ult_last_trans_n']} = NO transmitidos ({datos['ult_pct_no_trans_last']} — usá EXACTAMENTE este porcentaje, no lo recalcules), tardío {datos['ult_pct_tardio_last']}. Total lastre: {datos['ult_last_tot_n']}. Variación vs mes anterior: {datos['var_last_trans_pp']} pp.
    ...
    if conclusion_ia:
        ...
        for lin in lineas_conc:
            ...
            es_artefacto = (
                ...
                (ls.startswith("**") and "conclusiones" in ls.lower() and "estado actual" in ls.lower())
            )
            if not es_artefacto:
