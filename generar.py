...
def correr_queries(ruta_db, pais, anio, mes_d, mes_h, log_fn):
    ...
    """, (pais, anio, per_ult))
    rech_ant = q("""WITH p AS (SELECT NroMic, MAX(strftime('%Y-%m',Fecha_ISO)) AS ult_mes FROM RECHAZOS WHERE PaisEmisor=? AND Anio=? AND Metodo='OficializarMicDta' AND strftime('%Y-%m',Fecha_ISO)=? GROUP BY NroMic) SELECT COUNT(*) AS total FROM p""", (pais, anio, per_ant))
...
Estilo: formal, t\u00e9cnico, espa\u00f1ol rioplatense. Sin markdown, sin t\u00edtulos. No repetir el nombre de la secci\u00f3n al inicio del texto. Evitar anglicismos (usar "desempe\u00f1o" en lugar de "performance", etc.).REGLAS ESTRICTAS DE FORMATO:
...
- Revisá la ortografía antes de responder. No uses gerundios mal formados ni palabras inventadas."""
        ...
        result = [p.strip().replace("---BLOQUE---","").strip() for p in msg.content[0].text.split("---BLOQUE---") if p.strip()]
        result = [p for p in result if p]
        # Limpiar encabezados sobrantes que la IA puede incluir
        ...
        def limpiar_encabezado(texto):
            ...
            while lineas and lineas[0].strip().rstrip(":").strip() in encabezados:
                lineas.pop(0)
            return "\n".join(lineas).strip()
        result = [limpiar_encabezado(p) for p in result]
        result = [p for p in result if p]
        return result
...
Transmisi\u00f3n: {datos['ult_trans']} transmitidos ({datos['ult_pct_trans']}), {datos['ult_no_trans']} no transmitidos ({datos['ult_pct_no_trans']}), {datos['ult_tardio']} tard\u00edos ({datos['ult_pct_tardio']})
  ...
  \u2014 Lastre: trans {datos['ult_pct_trans_last']} (es decir, {datos['ult_last_trans_n']} operaciones), no trans {datos['ult_pct_no_trans_last']}, tard\u00edo {datos['ult_pct_tardio_last']}\n  \u2014 Valores absolutos cargados del mes: trans={datos['ult_carg_trans_n']}, no_trans={datos['ult_carg_no_trans_n']}, tard\u00edo={datos['ult_carg_tardio_n']} de {datos['ult_carg_tot_n']} cargados
  — Lastre último mes: {datos['ult_last_trans_n']} transmitidos de {datos['ult_last_tot_n']} lastre totales. Si ult_last_trans_n es 0, escribí 'ninguna operación lastre fue transmitida' — NUNCA uses frases como 'por redondeo' o '0 movimientos por redondeo'.
Rechazos del mes: total={datos['ult_rechazos']}, duplicados={datos['ult_rech_duplicados']}, operativos reales={datos['ult_rech_operativos']}. Variación vs mes anterior: {datos['var_rech_dir']} de {datos['var_rech_abs']} ({datos['var_rech_pct']}). Usá EXACTAMENTE estos valores pre-calculados. NO calcules la variación por tu cuenta.
...
def _generar_word(pais, anio, mes_d, mes_h, version,
    ...
    if conclusion_ia:
        ...
        for lin in lineas_conc:
            ...
            es_artefacto = (
                ls in ["Conclusiones y estado actual", "CONCLUSIONES Y ESTADO ACTUAL",
                        "**Conclusiones y estado actual**"] or
                ls.startswith("— Circuito ") or
                ls.startswith("- Circuito ")
            )
            if not es_artefacto:
...
def generar_informe(ruta_db, pais, anio, mes_d, mes_h, usar_ia, api_key, carpeta, log_fn):
    ...
    if usar_ia and api_key and ANT_OK:
        ...
        conclusion_ia=generar_conclusion_ia({
            ...
            "ant_rechazos":fmt(total_rech_ant),
            "var_rech_abs":fmt(abs(ult_rech_total-total_rech_ant)),
            "var_rech_dir":"aumento" if ult_rech_total>total_rech_ant else "disminución",
            "var_rech_pct":(f"{round(abs(ult_rech_total-total_rech_ant)/total_rech_ant*100,0):.0f}%" if total_rech_ant>0 else "N/D"),"ev_tabla_texto":" | ".join(ev_tabla_lineas),
            "g_total":fmt(gTot),"g_trans":fmt(gT),"g_no_trans":fmt(gN),"g_tardio":fmt(gTd),
            "pct_trans":pct(gT,gTot),"pct_no_trans":pct(gN,gTot),"pct_tardio":pct(gTd,gTot),
